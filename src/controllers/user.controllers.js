...
const Register = asyncHandler(async (req, res, next) => {
  const { error, value: userInfo } = userSchema.validate(req.body ?? {}, {
    abortEarly: false,
  ...
  try {
    ...
    sendWelcomeMessage(userInfo.email, userInfo.fullname).then(() => {});
  } catch (error) {
...
const AuthWithGoogle = asyncHandler(async (req, res, next) => {
  ...
  if (!foundUser) {
    ...
    sendWelcomeMessage(user.email, user.fullname).then(() => {});
  }
...
const EditPassword = asyncHandler(async (req, res, next) => {
  ...
  if (!/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d\W_]{8,}$/.test(newPassword)) {
    throw new HTTPError(
      400,
      'Password too weak. 8 chars min, with upper,lower and number'
    );
  }
...
const HandleResetEmailPassword = asyncHandler(async (req, res, next) => {
  ...
  sendResetPasswordEmail(email, resetLink).then(() => {});
